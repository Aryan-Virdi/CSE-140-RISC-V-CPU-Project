...
int execute(int alu_intake_1, int alu_intake_2, int alu_control, int sign_extension_offset, int pc_plus_4, int& alu_zero, int& branch_target, int pc_current, bool is_jal, bool is_jalr, int& jump) {
    int alu_result = 0;
    ...
    if (is_jal) {
        jump = pc_current + sign_extension_offset;
    }

    if (is_jalr) {
        jump = (alu_intake_1 + sign_extension_offset) & ~1;
    }
